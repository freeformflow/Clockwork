...
<html>
<head>
<title>Adjust Crew Competencies (Crew-Centric)</title>
...
<body>
	<!-- This page pulls the competencies for this crew member from the database and allows the User to edit them with the form.  This interface is "crew-centric" meaning it is easy to add many roles to a single crew member.  We need to use PHP to create the form inputs and set their starting values according to what is already stored. --!> 
	
	<div id="background">
	<div id="dialogContent">
				<h2>Adjust Crew Competencies (Crew-Centric)</h2>
			<form action="SubmitCrewCentricComptencies.php" method="post">
				...
				<?php
					...
					// Get employeeID
					$result = mysqli_query($con, 'select employeeID from crew where firstName="' . $firstName . '" and lastName="' . $lastName . '"');
					$result = mysqli_fetch_array($result);
					$employeeID = $result[0];
					
					echo '<input type="hidden" name="employeeID" value="'. $employeeID . '">'; 
					
					echo '<b>' . $lastName . ', ' . $firstName . '</b><br><br>';
			...
			// Pull data from *pins*
			$result = mysqli_query($con, "select * from pins where employeeID=$employeeID");
			...
			// Pulling names from *roleList*, we can generate alphabetcial rosters of roles in each category.
		...
		?>
				 ...
				 The current list of pins this crew member possesses is below.  Please make any necessary adjustments and submit the new competencies to the database.
				 
				 <hr>
		
	<div id="unPinnedSection">
	<h3> Un-Pinned Roles </h3>
